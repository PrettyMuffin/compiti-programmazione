...
{
    // sono andato fuori percorso (0 trovato nella matrice)
    if (pos_x < 0 || pos_y >= dim_y || pos_x >= dim_x || pos_y < 0)
        return 0;
    // sono sopra un fiore, dato è maleducato pestarci sopra, evito il passaggio
    if (!campo[pos_x][pos_y])
        return 0;
    // sono arrivato alla fine del campo (prima riga), e controllo se posso
    // passare
    if (pos_x == 0 && campo[pos_x][pos_y])
        return 1;
