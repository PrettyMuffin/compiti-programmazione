...
struct nodo *insert(struct nodo *ptr, int pos, int info)
{
    ...
    if (pos == 0)
    {
        struct nodo *newNodo = new_nodo(info);
        newNodo->next = ptr;
        return newNodo;
    }
    struct nodo *primo_elemento = ptr;
    // inserimento qualunque posizione
    for (; pos > 1; pos--)
        ptr = ptr->next;
    struct nodo *newNodo = new_nodo(info);
    newNodo->next = ptr->next;
    ptr->next = newNodo;
...
struct nodo *delete(struct nodo *ptr, int pos)
{
    ...
    if (pos == 0)
    {
        struct nodo *appoggio = ptr->next;
        free(ptr);
        return appoggio;
    }
    // eliminazione qualunque posizione
    struct nodo *primo_elemento = ptr;
    for (; pos > 1; pos--)
        ptr = ptr->next;
    struct nodo *appoggio = ptr->next->next;
    free(ptr->next);
    ptr->next = appoggio;
    return primo_elemento;
}
